...
# experiment: drop first and last 2 quantiles
dset2 <- subset(dset2, as.numeric(quantile) >= 3 & as.numeric(quantile) <= 7)
dset2$quantile <- factor(dset2$quantile)
...
quantile_regressions(A.tasks, B.tasks,
                     ...
                     out="analysis/quant_reg_i_experiment",
                     label="tab:quantreg1"
                     )
