...
A.tasks <- merge(x=A.df, y=tasks.combinedi,
                 by.x='occupation', by.y='COMBINED1')
A.tasks$Population <- pop82$population

B <- cbind(B, occupation=1:nrow(B))
names(B) <- c("B", "occupation")
B.tasks <- merge(x=B, y=tasks.combinedi,
                 by.x='occupation', by.y='COMBINED1')
B.tasks$Population <- pop82$population
...
write.csv(A.results, file='/tmp/A.i.results.csv')
write.csv(B.results, file='/tmp/B.i.results.csv')
